...
- 支持逐段「接受润色」或「保留原文」
- 支持多次润色：可点击「再次润色」或在右侧对话中提出新要求后继续润色
- 润色历史与回滚：每轮润色自动保存快照，可回滚到上一轮或指定历史版本
...
3. 对每段点击 **接受润色** 或 **保留原文**
4. 需要继续优化时，点击 **再次润色**，或在右侧输入额外要求（如「改为学术风格」）后点击 **发送**
5. 若对某轮结果不满意，使用 **回滚上一轮** 或 **回滚到指定版本** 恢复之前的润色状态
6. 完成后点击 **导出最终文章**，复制或下载结果
...
- 首次润色、每次「再次润色」及右侧发送消息都会调用 DeepSeek API，请注意用量
- 回滚会丢弃当前轮次之后的润色历史；回滚后若再次润色，将从回滚版本继续生成新一轮
